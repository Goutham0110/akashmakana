...
Produce ready-to-ship, production-grade code, not prototypes. No shortcuts unless asked. Don't stop until the implementation is complete (beautiful, responsive, fast, precise, bug-free, on brand). Battle-test every page/section/component with the tools available (browser screenshotting, computer use).
...
The skill has 24 sub-commands (`craft`, `shape`, `audit`, `critique`, `polish`, `bolder`, `quieter`, `distill`, `harden`, `onboard`, `animate`, `colorize`, `typeset`, `layout`, `delight`, `overdrive`, `clarify`, `adapt`, `optimize`, `init`, `document`, `extract`, `live`) plus `pin`/`unpin`.

**Decide first, then load the menu only if needed:**

- **General design request, no command word** (e.g. "design the onboarding screen", "make this flow clearer") → do NOT read the commands reference. Apply the General rules above and the loaded register reference, using the request as context.
- **Explicit command, or intent that clearly maps to one** (e.g. "fix the spacing" → `layout`, "rewrite this error" → `clarify`), **or a no-argument "what should I do?"** → read [reference/commands.md](reference/commands.md) for the full command table and routing rules, then follow them. The first word is the command; the rest is the target.

Setup (context, register) is already loaded by then; sub-commands don't re-invoke `/impeccable`. `teach` is a deprecated alias for `init`.
